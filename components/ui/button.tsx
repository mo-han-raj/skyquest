...
// Define the button styles with variants
  ...
  "inline-flex items-center justify-center whitespace-nowrap rounded-full text-base font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        shimmer: "relative overflow-hidden text-primary-foreground bg-primary", // Shimmer variant
      },
      size: {
...
// Forward ref and apply shimmering effect
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return (
      ...
      >
        {variant === "shimmer" && (
          <div className="absolute top-0 left-0 h-full w-full bg-gradient-to-r from-transparent via-white to-transparent opacity-30 animate-shimmer" />
        )}
        {props.children}
      </Comp>
    );
